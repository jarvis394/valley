...
export default class TusProvider {
  ...
  makeTusServer() {
    ...
    switch (storageDriver) {
      case 'gcs':
        datastore = TusProvider.makeGCSStore()
        break
      case 'fs':
        datastore = TusProvider.makeFSStore()
        break
      case 's3':
        datastore = TusProvider.makeS3Store()
        break
      default:
    ...
    return new Server({
      path: TusProvider.TUS_ENDPOINT_PATH,
      datastore,
      allowedCredentials: true,
      respectForwardedHeaders: true,
      allowedOrigins: ALLOWED_ORIGINS,
      maxSize: MAX_UPLOAD_FILE_SIZE,
      namingFunction: TusProvider.namingFunction,
      getFileIdFromRequest: TusProvider.getFileIdFromRequest,
  ...
  handlUploadCreate: ServerOptions['onUploadCreate'] = async (
    ...
    const folderService = await this.app.container.make(FolderService)
    ...
    const uploadFolder = await folderService.folder({
      where: {
  ...
  handleUploadFinish: ServerOptions['onUploadFinish'] = async (
    ...
    const fileService = await this.app.container.make(FileService)
   ...
   */
  register() {
    this.app.container.singleton('tus:server', async () => {
      return this.makeTusServer()
    })
  }
}
